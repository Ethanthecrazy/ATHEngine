...
int ATHAudio::SFXLoadSound( std::string _szSoundName, const TCHAR* szFileName )
{
	...
	if( nCapture > -1 )
		m_mapNameLibrary[ _szSoundName ] = nCapture;
...
int ATHAudio::SFXGetSoundID( std::string _szSoundName )
{
	...
	{
		return m_mapNameLibrary[ _szSoundName ];
	}
...
void ATHAudio::SFXUnloadSound( int nID )
{
	...
	std::map< std::string, int >::iterator itrNames = m_mapNameLibrary.begin();

	while( itrNames != m_mapNameLibrary.end() )
	{
		if( (*itrNames).second == nID )
		{
			m_mapNameLibrary.erase( itrNames );
			break;
		}
